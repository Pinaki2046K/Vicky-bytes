...
import { MOCK_MESSAGES, generateRandomMessage, type ChatMessage } from '@/data/chat'
  ...
  return (
    <div className="flex flex-col h-full bg-surface-1 rounded-2xl border border-white/5 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 flex-shrink-0">
        ...
        <span className="text-xs text-ink-muted font-mono">{messages.length} msgs</span>
      </div>
      ...
      {/* Input */}
      <form onSubmit={sendMessage} className="px-3 py-3 border-t border-white/5 flex-shrink-0">
        <div className="flex items-center gap-2 bg-surface-2 border border-white/8 rounded-xl px-3 py-2 focus-within:border-brand-500/40 transition-colors">
          ...
          />
          <button type="button" className="text-ink-muted hover:text-ink transition-colors p-0.5">
            <Smile size={14} />
          </button>
          <button
            type="submit"
        ...
        <p className="text-[9px] text-ink-faint mt-1.5 px-1">
          {isLive ? 'Chat is live — be respectful 👋' : 'Chat is paused for offline events'}
        </p>
      </form>
    </div>
  )
